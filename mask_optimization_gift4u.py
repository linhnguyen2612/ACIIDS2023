...
class DistributedAdamMultiTasks(Optimizer):
    ...
    def __init__(self, params, lr=required, warmup=-1, t_total=-1, schedule='warmup_linear', b1=0.9, b2=0.999, e=1e-6, weight_decay=0.01, max_grad_norm=1.0):
        ...
        super(DistributedAdamMultiTasks, self).__init__(params, defaults)
    def get_lr(self):
        lr = []
    ...
    def step(self, closure=None, t=None, id_to_mask_params = None):
        ...
        for group in self.param_groups:
            for p_id,p in enumerate(group['params']):
                ...
                check = id_to_mask_params is not None
                if check:
                    check = check & (p_id in id_to_mask_params)
                if check:
                    #print(update)
