...
def logloss(ground_truth, prediction):
    ...
    return logloss
def pcount_at_k_batch(rank_indices, k, items_unpopularity):
    items_popularity = 1 - items_unpopularity
    items_populariy_with_rank_indices = items_popularity[rank_indices[:, :k].reshape(-1)].reshape(-1, k)
    return np.mean(np.mean(items_populariy_with_rank_indices, axis = -1), axis = -1)

def calc_metrics_at_k(cf_scores, train_user_dict, test_user_dict, user_ids, item_ids, Ks, items_out_task = None, items_unpopularity = None):
    """
    ...
    for idx, u in enumerate(user_ids):
        ...
        try:
            ...
            continue
        if items_out_task is not None:
            cf_scores[idx][items_out_task] = -np.inf
    try:
    ...
    for k in Ks:
        ...
        try:
            metrics_dict[k]['pcount']      = pcount_at_k_batch(rank_indices, k, items_unpopularity = items_unpopularity)
        except:
            continue
    return metrics_dict

def calc_metrics_at_k_without_masking(cf_scores, train_user_dict, test_user_dict, user_ids, item_ids, Ks, train_data_masking_test_file, items_out_task = None, items_unpopularity = None):
    """
    cf_scores: (n_users, n_items)
    """
    test_pos_item_binary = np.zeros([len(user_ids), len(item_ids)], dtype=np.float32)
    for idx, u in enumerate(user_ids):
        ...
        test_pos_item_binary[idx][test_pos_item_list] = 1
        try:
            train_pos_item_list = train_data_masking_test_user_dict[u]
            cf_scores[idx][train_pos_item_list] = -np.inf
        except:
            continue
        if items_out_task is not None:
